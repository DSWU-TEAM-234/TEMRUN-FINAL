...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    private val windowSizeMillis = 4000L                // 예측에 사용할 윈도우 크기: 5초
    ...
    var latestPredictedCadence: Int = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        endButton.setOnClickListener {
            ...
            val cadenceData = ArrayList<Int>()

            val intent = Intent(this, ResultActivity::class.java).apply {
                putIntegerArrayListExtra("cadenceHistory", ArrayList(predictionHistory))
                putParcelableArrayListExtra("songs", playedTracks)
                putExtra("time", elapsedSeconds)
                putExtra("calorie", calorie)
                putExtra("distance", distance)
                putExtra("averageBPM", bpmList.average().toInt())
            }
    ...
    //케이던스 예측 루프 - CadenceFragment와 연동 추가
            ...
            if (recentData.isNotEmpty()) {
                executor.execute {
                    ...
                    runOnUiThread {
                        ...
                        latestPredictedCadence = finalPrediction

                        Log.d("DataFlow", "전달된 케이던스: $finalPrediction")
                        val targetCadence = currentBpm // 목표 케이던스 값
                        val now = System.currentTimeMillis()
                        if (kotlin.math.abs(latestPredictedCadence - targetCadence) >= 20) {
                            // 4초(4000ms) 쿨다운 체크
                            if (now - lastVibrationTime >= 4000) {
                                triggerVibration()
                                showCadenceFeedbackToast(this@MainActivity)
